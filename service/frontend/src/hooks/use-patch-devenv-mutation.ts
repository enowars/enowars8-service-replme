...
export function usePatchDevenvMutation(options: PatchDevenvMutationOptions) {
  ...
  return useMutation({
    mutationFn: (patch: PatchDevenvRequest) => axios.patch<void>(
    ...
    onSuccess: (_, data) => {
      ...
      queryClient.setQueryData<Devenv>(
        ['devenv', options.uuid],
        (oldData) => {
          if (!oldData)
            return oldData;
          ...
          if (data.runCmd && data.runCmd !== "")
            oldData.runCmd = data.runCmd
          if (options.onSuccess)
            options.onSuccess(oldData)
          return { ...oldData }
        }
      )
    },
