...
export function useDevenvGenerationMutation(options: DevenvGenerationMutationOptions) {
  ...
  return useMutation({
    mutationFn: async (gen?: number | null) => {
      if (gen === null) {
        ...
        let generation = queryClient.getQueryData<number>(['devenv', options.uuid, 'generation'])
        generation = generation ? generation + 1 : 1;
        queryClient.setQueryData(['devenv', options.uuid, 'generation'], generation)
      }
    }
  });
}
