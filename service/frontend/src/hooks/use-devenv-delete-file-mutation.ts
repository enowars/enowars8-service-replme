...
export function useDevenvDeleteFileMutation(options: DevenvDeleteFileMutationOptions) {
  ...
  return useMutation({
    mutationFn: (filename: DeleteFilePayload) => axios.delete(
      (process.env.NEXT_PUBLIC_API ?? "") + '/api/devenv/' + options.uuid + "/files/" + encodeURI(filename),
      {
      ...
      queryClient.setQueryData<string[]>(
        ...
        (oldData) => {
          ...
          if (options.onSuccess)
            options.onSuccess(filename, _data)
          return _data
        }
      )
    }
